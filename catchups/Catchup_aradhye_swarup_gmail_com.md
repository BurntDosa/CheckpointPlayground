...
You missed a flurry of foundational commits introducing three major new systems: **SplitFM** (a privacy-preserving fine-tuning/inference framework for LLMs), a **React frontend** (Dockerized with Nginx), and a **PDF Reader with LLM** (FastAPI + MongoDB + Qdrant for semantic search). No existing code was modified—these are all greenfield additions. Your immediate focus should be on the breaking changes in SplitFM’s LoRA integration and the new Docker-based infrastructure for the frontend/PDF services.
